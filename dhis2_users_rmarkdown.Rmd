...
#credentials and global variables (include last backslash)
baseurl<-"https://play.dhis2.org/2.31.1/"
username<-"admin"
password<-"district"
...
viewName1 <- "test_user_data_withstartdate"
...
if(is.null(viewUID)){
  print("no SQL view with this name exists. Creating new SQL view.")
  ...
  print(paste0("SQL view with this name already exists...",viewName1))
}
file.exists(paste0(wd, "/",viewName1,".json"))

#now execute the SQL view
createsql<-paste0(baseurl,"api/26/sqlViews/",viewUID,"/execute")
r<-httr::POST(createsql, httr::authenticate(username,password),httr::timeout(60))
if(r$status_code == 200){
  print(paste0("executing view.. ",viewName1))
} else {
  print(paste0("could not execute view ",viewName1))
}
...
r<-httr::GET(url, httr::authenticate(username,password),httr::timeout(60),write_disk("response.txt",overwrite = TRUE))

if(r$status_code == 200){
  print("pulled TEI audit data to R")
}

mydata<-readr::read_delim("response.txt",delim=",", col_names = TRUE, col_types = "cdci")
...
head(na.omit(latest_users[,c(3,7)]))
...
The five program stages with most user interactions are below.
```{r Top Stages, fig.height=3}

#note: "interactions" abbreviated to "edits" in analysis code.

cs<-as_data_frame(aggregate((df$edits), list(Stage = df$stagename), sum))
most_common<-head(cs[order(cs$x, decreasing = TRUE),],5)
names(most_common)[2]<-"Interactions"
most_common<-tableGrob(most_common, theme =ttheme_minimal(padding=unit(rep(2,2),"mm")))
gridExtra::grid.arrange(most_common, as.table=FALSE)
```
...
get_p2<-function(ub_agg2){
 p2<- ggplot(data=ub_agg2, aes(x=hour, y=edits, fill=stagename)) +
  ...
  geom_vline(xintercept=7.5, size=1, color="firebrick") +
  geom_vline(xintercept=16.5, size=1, color="firebrick") +
...
groups_auditted<-length(usergroups2$displayName)

#create empty table for groups by interactions
ugroup_table<-data.frame(group=character(),interactions=integer())
...
ub<-df[df$user %in% group_users, ]
  if(length(ub$user) && sum(ub$edits)>0){
    ...
    groupname_edits<-data.frame(group=groupname,interactions=sum(ub_agg1$edits))
...
common_groups<-data.frame(head(ugroup_table[order(ugroup_table$interactions,decreasing = TRUE),],5))
common_groups_table<-tableGrob(common_groups, theme=ttheme_minimal(padding=unit(rep(2,2),"mm")))

gridExtra::grid.arrange(common_groups_table)
```
...
user_edits_table<-data.frame(user=character(),interactions=integer())
...
user_edits<-data.frame(user=value,interactions=sum(ub$edits))
...
#create the table of common groups
common_users<-data.frame(head(user_edits_table[order(user_edits_table$interactions,decreasing = TRUE),],5))
common_users_table<-tableGrob(common_users,theme=ttheme_minimal(padding=unit(rep(2,2),"mm")))

gridExtra::grid.arrange(common_users_table)
```
