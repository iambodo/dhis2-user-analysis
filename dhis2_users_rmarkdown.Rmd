---
title: "DHIS2 Tracker: User Activity Analysis"
output:
  html_document:
    toc: true
    toc_float: true
...
baseurl<-"http://hrhr.dhis2.org/dhis/"
username<-"brian"
...
#if blank, will use today's date as start date, and 1 year ago today as end date
...
required_packages <- c("ggplot2", "stringr","httr","jsonlite","assertthat","rlist","readr",         "tibble","plyr","tidyr","dygraphs","xts","gridExtra","cowplot","lubridate","plotly","askpass","DT","keyring","devtools")
...
With these data, we can analyze user activity with tracker data elements, referred to as *edits* in our tables below.
...
```{r pull data by API, echo=FALSE, message=FALSE}
#Here we'll see if we can pull data directly from tei API and manipulate

####priorities
#get all tables through API - done

#tab for users and tab for user groups - done
#time series by hour, with lines per day of week 0- done

#apply these data to the charting and DT functions -  done
#folders for users and user groups graphics - done

#heatmap by stage x hour
#link the data table and the heatmap with link to users list - send this as stackoverflow question

#"sessions" = time working on a program stage - done
...
#better method: paginate the response
url<-paste0(baseurl,"api/audits/trackedEntityDataValue.json?paging=true&pageSize=10000")
pages_array<-fromJSON(content(GET(url),"text"), flatten = TRUE)[[1]]
pages_list<-pages_array$pageCount
paged_resp<-list()

#debug into the loop: print(paste("Retrieving page ", i))

for(i in 1:pages_list){
  url_paged<-paste0(url, "&page=", i)
  paged_resp[[i+1]]<-jsonlite::fromJSON(content(GET(url_paged),"text"), flatten = TRUE)[[2]]
}

all_tei_audits<-rbind_pages(paged_resp)
...
An update may be a _new_ data value being entered into a stage, _or_ editing an existing value. A deletion is an erasure of an existing data value. For simplicity we can refer to these together as *edits*.

See the [DHIS2 developer guide](https://docs.dhis2.org/master/en/developer/html/dhis2_developer_manual_full.html) for details.
...
total_users<-length(unique(users$displayName))
total_loggedin<-length(unique(na.omit(users$userCredentials.lastLogin)))
...
ex2<-sample(top_users$username, 1)


```

##User Details - Who Enters Tracker Data?
...
##User Audits by Stage
We can extrapolate from this which program stages show the most frequent edits.
...
Heat map by stage and hour that TEI edits were recorded. 
Only shows the top 20 stages of edits, all others are grouped as "OTHER"
...
```{r overall charts, echo=FALSE, fig.align='center', fig.show='hold'}
...
```{r chart by hour and time of day, echo=FALSE}
...
Graph showing top stages by hour for each user group are found at directory `r getwd()` E.g. at `r paste0(getwd(),"/plots/usergroups")`

Below are the user groups with the most user interactions ("edits") in tracker. 

```{r top user groups, echo=FALSE,message=FALSE, fig.height=2}
...
bsselectR::bsselect(usergroup_plots, type = "img", selected = usergroup_plots[1], 
         live_search = TRUE, show_tick = TRUE)

#create the table of common user groups

DT::datatable(edits_by_usergroup_all, extensions = 'Buttons', options = list(
...
```{r top users, echo=FALSE, eval=TRUE, fig.height=2}
...
bsselectR::bsselect(users_plots, type = "img", selected = users_plots[1], 
         live_search = TRUE, show_tick = TRUE)
...
DT::datatable(edits_by_user_all, 
              caption = "All users by most tracker edits",
              extensions = 'Buttons', 
              options = list(
                pageLength = 5,
                dom = 'Blfrtip', 
...
Here we convert the TEI audit log to show time between first and last auditted change.
this should give a sense of quality as well.
...
```{r stage sessions, echo=FALSE, fig.align='center',}
...
`r length(psi_users$name)` events had multiple users submit data. This may be evidence of data tampering!
...
```{r test crap, eval=FALSE, echo=FALSE}
#comment out this test
#edits_by_userGroup<-edits_by_user %>%
 # left_join(users, by = c("modifiedBy" = "userCredentials.username")) %>%
#  filter(modifiedBy %in% ugroups$users[10])

##crosstalk doesnt look like it will work with heatmap!
library(crosstalk)
library(DT)
library(d3heatmap)

d<-edits_by_user_stage_dayhour %>%
  ungroup() %>%
  group_by(displayName, hour) %>%
  arrange(desc(displayName,edits)) %>%
  select("displayName", "hour","edits")


d$edits<-as.numeric(d$edits)
...
#sd<-SharedData$new(d)
#hlight<-highlight_key(sd)

#dtab<-datatable(sd, caption = "Users with most frequent interactions")
#p<-plotly::plot_ly(sd, 
                   x=~hour,y=~displayName,z=~edits, 
                   colorscale = "Grey", type="heatmap", autosize=TRUE)

bscols(p, dtab, widths = 12)


#gtitle_names<-gsub("[^A-Za-z0-9,;._-]","",usergroups_list)
#gtitles_full<- paste0("usergroup_",gtitle_names,".png")

#purrr::pwalk(list(gtitles_full, plots), ggsave, path = "plots", dpi = 500)
#pwalk(list(paths, plots), ggsave, path = "plots", dpi = 300)

#can we pre-select a filter in data table?? then make 
